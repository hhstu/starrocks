...
In the Primary Key table, the primary key and the sort key are coupled, which are specified by `PRIMARY KEY` and are assigned UNIQUE and NOT NULL constraints.
